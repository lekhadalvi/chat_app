...
export default function Home() {
  ...
  const handleScreenClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    for (let i = 0; i < count; i++) {
      // Angle spread and offset from exact click point
      ...
      newParticles.push({
        ...
        size: Math.floor(Math.random() * 12) + 20, // 20px to 32px
        rotStart: Math.random() * 30 - 15, // -15deg to 15deg
        rotEnd: Math.random() * 50 - 25,  // -25deg to 25deg
        wobbleDist: Math.random() * 24 - 12, // -12px to 12px
      });
    }
  ...
  return (
    <div
      ...
      className="min-h-screen w-full relative overflow-x-hidden flex flex-col items-center justify-between p-6 select-none font-comic text-black"
      style={{ cursor: "pointer" }}
    >
      {/* Pink lightning bolt (Top-Left decoration) */}
      <div className="absolute top-6 left-6 md:top-12 md:left-16 w-16 h-20 md:w-24 md:h-28 text-zap-pink opacity-85 transform -rotate-12 select-none pointer-events-none transition-transform duration-300 hover:rotate-6">
        <svg
          viewBox="0 0 100 100"
      ...
      <div className="absolute bottom-6 right-6 md:bottom-16 md:right-20 w-16 h-16 md:w-24 md:h-24 text-[#C4E8E2] opacity-85 transform rotate-12 select-none pointer-events-none transition-transform duration-300 hover:-rotate-12">
        <svg
          viewBox="0 0 100 100"
          className="w-full h-full filter drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]"
          fill="currentColor"
      ...
      <div className="flex items-center gap-5 my-6 select-none relative z-25">
        {/* Yellow Button (Chat bubble icon) */}
        <button
